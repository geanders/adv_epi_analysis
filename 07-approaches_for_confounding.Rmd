...
```{r}
fhs_first <-fhs_first %>%
  mutate(p.smok.obs=ifelse(cursmoke == 0,   ###estimate the conditional probability that someone is unexposed or exposed from the model above
...
fhstv_incMI <-fhstv_incMI %>%
  ...
  mutate(p.smok.obs=ifelse(cursmoke == 0,   ###estimate the conditional probability that someone is unexposed or exposed from the model above
         ...
         w_i=1/p.smok.obs,#### This is the first step
...
fhstv_incMI <-fhstv_incMI %>%
  group_by(randid) %>%
  mutate(p.smok.obsnum=ifelse(cursmoke == 0,   ###estimate the conditional probability that someone is unexposed or exposed from the model above
         1 - predict(mod_tvIPWnum, type = "response"),
         predict(mod_tvIPWnum, type = "response")),
         w_i=p.smok.obsnum/p.smok.obs,#### This is the first step
         w=cumprod(w_i)) %>% ### the second step gives us the cumulative product of the inverse probabilities over   time. 
         ungroup()
