...
\tightlist
\item
  \textbf{Fit a conditional Cox model for the effect of smoking on mortality adjusting for sex and age using the limited dataset we created in the previous chapter (one observation per participant, \texttt{fhs\_first}). Estimate inverse probability weights for smoking exposure and fit a Marginal Structural Cox model using the weighted population. How do the results of the two models compare?}
\end{enumerate}
...
One way we can use these propensity scores is through \emph{inverse probability weighting} or IPW. IPW allows us to adjust for confounding without having to include additional variables in our final outcome model other than the exposure of interest. Instead, we weight each participant by the inverse of the probability that they had the exposure value they indeed had conditional on these other (potentially confounding) variables. The quantity we are estimating is:
...
\NormalTok{fhstv\_incMI }\OtherTok{\textless{}{-}}\NormalTok{ fhstv\_incMI }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{ungroup}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{filter}\NormalTok{(}\SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(cursmoke\_l1) }\SpecialCharTok{\&} \SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(bmi\_l1) }\SpecialCharTok{\&} \SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(sysbp\_l1))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{                                cursmoke }\SpecialCharTok{+}\NormalTok{ age }\SpecialCharTok{+}\NormalTok{ sex, }
                          \AttributeTok{weights =}\NormalTok{ w\_new, }\AttributeTok{cluster =}\NormalTok{ randid,}
                          \AttributeTok{data =}\NormalTok{ fhstv\_incMI)}
...
\item
  ...
  measures outcome in the Framingham family of studues (in this case, lung
...
grouping, and in these cases observations are typically likely to be more
...
models. We'll use the Framingham Heart Study data. Before, we looked at
outcomes in this dataset for which there was only one observation per study
subject. For example, we looked at the time from the baseline examination until
death, and there can only be one measurement (or censoring) for each person
for that outcome. Here, we'll look at a value that is measured at each
examination---blood pressure. We have repeated measures of blood pressure
for each study subject, and we'll investigate how those are associated with
...
\NormalTok{fhs\_sample }\OtherTok{\textless{}{-}}\NormalTok{ fhs }\SpecialCharTok{\%\textgreater{}\%} 
  ...
  \FunctionTok{select}\NormalTok{(randid, sex, age, sysbp,cursmoke, bmi, educ, time) }\SpecialCharTok{\%\textgreater{}\%} 
...
\begin{verbatim}
## # A tibble: 45 x 8
##     randid sex      age sysbp cursmoke   bmi  educ  time
##      <dbl> <fct>  <dbl> <dbl>    <dbl> <dbl> <dbl> <dbl>
##  1 7112885 female    37   126        0  25.9     2     0
##  2 7112885 female    43   126        0  28.5     2  2113
##  3 7112885 female    49   130        0  29.2     2  4327
##  4 9356685 male      64   176        0  24.9     3     0
##  5 9356685 male      70   174        0  22.4     3  2129
##  6 9356685 male      75   190        0  27.0     3  4164
##  7 4452358 female    59   141        0  25.3     1     0
##  8 4452358 female    65   165        0  27.4     1  2104
##  9 4452358 female    71   154        0  26.4     1  4286
## 10 9694232 female    48   155        0  28.5     1     0
## # ... with 35 more rows
\end{verbatim}
...
specify---you can use the \texttt{glm} functionm since we're using only a fixed effect
(more later on what that is, but essentially it's the type of effect we've
been fitting in all the previous models), and in the model formula, we only
need to specify \texttt{1} for an intercept:
...
randomly varies from the overall one. We can see that these subject-specific
intercepts range from the 120s through the 180s.
...
You can see from this section that the total variance is 333.2 + 151.8 = 485.
A lot of this (333.2 / 485 = 0.69, or about 69\%) results from variation between
subjects, but there's still some variance that comes from within-subject
variation (variation in blood pressure measurements from one examination to
...
standard deviation column given in the summary output for the model; if you want
to get the variances, you could just square these values.
...
\begin{verbatim}
## # A tibble: 45 x 19
##     randid sex      age sysbp cursmoke   bmi  educ  time .fitted  .resid  .hat
##      <dbl> <fct>  <dbl> <dbl>    <dbl> <dbl> <dbl> <dbl>   <dbl>   <dbl> <dbl>
##  1 7112885 female    37   126        0  25.9     2     0    130.  -3.51  0.292
##  2 7112885 female    43   126        0  28.5     2  2113    130.  -3.51  0.292
##  3 7112885 female    49   130        0  29.2     2  4327    130.   0.492 0.292
##  4 9356685 male      64   176        0  24.9     3     0    175.   0.767 0.292
##  5 9356685 male      70   174        0  22.4     3  2129    175.  -1.23  0.292
##  6 9356685 male      75   190        0  27.0     3  4164    175.  14.8   0.292
##  7 4452358 female    59   141        0  25.3     1     0    152. -11.1   0.292
##  8 4452358 female    65   165        0  27.4     1  2104    152.  12.9   0.292
##  9 4452358 female    71   154        0  26.4     1  4286    152.   1.92  0.292
## 10 9694232 female    48   155        0  28.5     1     0    146.   9.00  0.292
## # ... with 35 more rows, and 8 more variables: .cooksd <dbl>, .fixed <dbl>,
## #   .mu <dbl>, .offset <dbl>, .sqrtXwt <dbl>, .sqrtrwt <dbl>, .weights <dbl>,
## #   .wtres <dbl>
\end{verbatim}
...
variance components from both the between-subject variation (``randid'') and from
``left over'' variation after accounting for study subject (``Residual''). If you
compare these values to the intercept-only mixed model, you can see that the
variance associated with the study subject is now lower. This is because some
of the original variation in blood pressure from one study subject to another
was related to variation in age among study subjects. Now that we have a fixed
term for age, we've managed to use that to explain some of the variation
in blood pressure when comparing different study subjects (although there's
...
\texttt{educ} fixed effect term. Based on the estimate, it looks like systolic blood
...
slopes for how blood pressure changes with age. You can see that, for some
study subjects, this slope is very close to zero (i.e., blood pressure stays
similar with age), while for others, it's much higher than the fixed effects
estimate for the age slope of 0.72 from our earlier model.
