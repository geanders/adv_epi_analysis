...
\item
  \textbf{Construct a longitudinal dataset with repeated (time-varying) values for death and cardiovascular outcomes.}
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{fhstv<-fhs }\OperatorTok{%>%}
...
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{time_next=}\KeywordTok{lead}\NormalTok{(time), }\CommentTok{### bring forward the time from the next period}
         \DataTypeTok{time2=}\KeywordTok{ifelse}\NormalTok{(}\KeywordTok{is.na}\NormalTok{(time_next)}\OperatorTok{==}\NormalTok{F, time_next}\DecValTok{-1}\NormalTok{, timedth), }\CommentTok{### create a variable indicating the last day of follow-up in a given period}
         \DataTypeTok{deathtv=}\KeywordTok{ifelse}\NormalTok{(death}\OperatorTok{==}\DecValTok{1} \OperatorTok{&}\StringTok{ }\NormalTok{timedth}\OperatorTok{<=}\NormalTok{time2, }\DecValTok{1}\NormalTok{, }\DecValTok{0}\NormalTok{),}
         \DataTypeTok{timemi2=}\KeywordTok{ifelse}\NormalTok{(time2}\OperatorTok{<=}\NormalTok{timemi, time2, timemi),}
         \DataTypeTok{hospmitv=}\KeywordTok{ifelse}\NormalTok{(hospmi}\OperatorTok{==}\DecValTok{1} \OperatorTok{&}\StringTok{ }\NormalTok{timemi}\OperatorTok{<=}\NormalTok{timemi2,}\DecValTok{1}\NormalTok{,}\DecValTok{0}\NormalTok{),}
         \DataTypeTok{agetv=}\NormalTok{age}\OperatorTok{+}\NormalTok{(time}\OperatorTok{/}\FloatTok{365.25}\NormalTok{)) }\OperatorTok{%>%}\StringTok{ }\CommentTok{###create a time-varying age variable for age at beginning of each period}
...
\tightlist
\item
  \textbf{Fit a Cox model for the effect of smoking on mortality and MI hospitalizations using this dataset. How do the results compare to the results from the models in 7.3}
\end{enumerate}

Using the dataset with the time-varying outcomes we created above, we will repeat the same model for death (and MI hospitalizations) as a function of smoking and age and sex. However, unlike those models in section 7.3 we are not interested in the hazard occurring between beginning and end of follow-up for each participant, but rather the hazard occurring in each period. In order to address this we will slightly modify the \texttt{Surv} function in our model to accommodate the specific interval for each period, by designating two time variables (\texttt{time} and \texttt{time2}).

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{coxph_modtv1<-}\KeywordTok{coxph}\NormalTok{(}\KeywordTok{Surv}\NormalTok{(time, time2, deathtv)}\OperatorTok{~}\NormalTok{cursmoke}\OperatorTok{+}\NormalTok{agetv}\OperatorTok{+}\NormalTok{sex, }\DataTypeTok{data=}\NormalTok{fhstv)}
...
## 3 sex       -0.537    0.0517     -10.4  2.52e- 25
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{coxph_modhospmitv1<-}\KeywordTok{coxph}\NormalTok{(}\KeywordTok{Surv}\NormalTok{(time, timemi2, hospmitv)}\OperatorTok{~}\NormalTok{cursmoke}\OperatorTok{+}\NormalTok{agetv}\OperatorTok{+}\NormalTok{sex, }\DataTypeTok{data=}\NormalTok{fhstv)}
\end{Highlighting}
\end{Shaded}
...
## 3 sex       -1.11     0.110      -10.1  6.69e-24
\end{verbatim}

There is a warning here indicating that not all out time variables in each observation are in agreement with the rule \texttt{time2\textgreater{}time1}. If that is not the case the Cox model cannot assess a hazard for a time interval with length \textless=0. In our dataset we are including periods after a participant has experienced an MI hospitalization. The \texttt{Surv} function will create missing values for the hazard for these observations, so the estimation of model parameters is not affected, however, we should limit the dataset to the person-time actually at risk for incdient MI hospitalization. Luckily the dataset includes a time-varying \texttt{prevmi} variable indicating a prevalent MI, which we can use to subset to the person time that is still at risk for an incident MI hospitalization.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{fhstv_incMI<-}\StringTok{ }\NormalTok{fhstv }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{filter}\NormalTok{(prevmi}\OperatorTok{==}\DecValTok{0}\NormalTok{) }

\NormalTok{coxph_modhospmitv2<-}\KeywordTok{coxph}\NormalTok{(}\KeywordTok{Surv}\NormalTok{(time, timemi2, hospmitv)}\OperatorTok{~}\NormalTok{cursmoke}\OperatorTok{+}\NormalTok{agetv}\OperatorTok{+}\NormalTok{sex, }\DataTypeTok{data=}\NormalTok{fhstv_incMI)}
\end{Highlighting}
\end{Shaded}
...
## 3 sex       -1.11     0.110      -10.1  4.41e-24
\end{verbatim}
...
\StringTok{  }\KeywordTok{filter}\NormalTok{ (timemi2}\OperatorTok{<=}\NormalTok{time) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{pull}\NormalTok{(randid)}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{adv_epi_analysis_files/figure-latex/unnamed-chunk-225-1.pdf}
...
\tightlist
\item
  \textbf{Construct an exposure variable representing the history of smoking (identify if someone is a current, former and never smoker at each period). Repeat the model from 2, using this exposure instead. What added information does this model give us?}
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{fhstv<-fhstv }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{group_by}\NormalTok{(randid) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{smoking=}\StringTok{'never'}\NormalTok{,}
         \DataTypeTok{smoking=}\KeywordTok{ifelse}\NormalTok{(cursmoke}\OperatorTok{==}\DecValTok{1}\NormalTok{, }\StringTok{'current'}\NormalTok{, smoking),}
         \DataTypeTok{smoking=}\KeywordTok{ifelse}\NormalTok{(}\KeywordTok{is.na}\NormalTok{(}\KeywordTok{lag}\NormalTok{(cursmoke))}\OperatorTok{==}\NormalTok{F }\OperatorTok{&}\StringTok{ }\KeywordTok{lag}\NormalTok{(cursmoke)}\OperatorTok{==}\DecValTok{1} \OperatorTok{&}\StringTok{ }\NormalTok{cursmoke}\OperatorTok{==}\DecValTok{0}\NormalTok{, }\StringTok{'former'}\NormalTok{, smoking),}
         \DataTypeTok{smoking=}\KeywordTok{ifelse}\NormalTok{(}\KeywordTok{is.na}\NormalTok{(}\KeywordTok{lag}\NormalTok{(cursmoke))}\OperatorTok{==}\NormalTok{F }\OperatorTok{&}\StringTok{ }\KeywordTok{lag}\NormalTok{(smoking)}\OperatorTok{==}\StringTok{'former'} \OperatorTok{&}\StringTok{ }\NormalTok{cursmoke}\OperatorTok{==}\DecValTok{0}\NormalTok{, }\StringTok{'former'}\NormalTok{, smoking),}
         \DataTypeTok{smoking=}\KeywordTok{factor}\NormalTok{(smoking, }\DataTypeTok{levels=}\KeywordTok{c}\NormalTok{(}\StringTok{'never'}\NormalTok{, }\StringTok{'current'}\NormalTok{, }\StringTok{'former'}\NormalTok{))) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{ungroup}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{coxph_modtv2<-}\KeywordTok{coxph}\NormalTok{(}\KeywordTok{Surv}\NormalTok{(time, time2, deathtv)}\OperatorTok{~}\NormalTok{smoking}\OperatorTok{+}\NormalTok{agetv}\OperatorTok{+}\NormalTok{sex, }\DataTypeTok{data=}\NormalTok{fhstv)}
...
## 4 sex             -0.528    0.0524     -10.1  7.32e- 24
\end{verbatim}
...
##   term              hr low_hr high_hr
##   <chr>          <dbl>  <dbl>   <dbl>
## 1 smokingcurrent  1.42   1.27    1.59
\end{verbatim}
...
\NormalTok{coxph_modtv2 }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{tidy}\NormalTok{()}\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(term }\OperatorTok{==}\StringTok{ 'smokingformer'}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{hr =} \KeywordTok{exp}\NormalTok{(estimate),}
         \DataTypeTok{low_ci =}\NormalTok{ estimate }\OperatorTok{-}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\NormalTok{std.error, }
         \DataTypeTok{high_ci =}\NormalTok{ estimate }\OperatorTok{+}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\NormalTok{std.error, }
         \DataTypeTok{low_hr =} \KeywordTok{exp}\NormalTok{(low_ci), }
...
##   term             hr low_hr high_hr
##   <chr>         <dbl>  <dbl>   <dbl>
## 1 smokingformer  1.09  0.929    1.29
\end{verbatim}
...
\NormalTok{coxph_modhospmitv3<-}\KeywordTok{coxph}\NormalTok{(}\KeywordTok{Surv}\NormalTok{(time, timemi2, hospmitv)}\OperatorTok{~}\NormalTok{smoking}\OperatorTok{+}\NormalTok{agetv}\OperatorTok{+}\NormalTok{sex, }\DataTypeTok{data=}\NormalTok{fhstv_incMI)}
...
## 4 sex             -1.12     0.111     -10.1   6.65e-24
\end{verbatim}
...
##   term              hr low_hr high_hr
##   <chr>          <dbl>  <dbl>   <dbl>
## 1 smokingcurrent  1.60   1.28    2.00
\end{verbatim}
...
##   term             hr low_hr high_hr
##   <chr>         <dbl>  <dbl>   <dbl>
## 1 smokingformer 0.920  0.639    1.32
\end{verbatim}
...
\textbf{Explore the role of smoking intensity on the hazard for MI hospitalizations, using the \texttt{cigpday} variable. What can we infer about the exposure-response?}

Let's repeat our latest model for smoking and MI hospitalizations including the \texttt{cigpday} variable

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{coxph_modhospmitv4<-}\KeywordTok{coxph}\NormalTok{(}\KeywordTok{Surv}\NormalTok{(time, timemi2, hospmitv)}\OperatorTok{~}\NormalTok{smoking}\OperatorTok{+}\NormalTok{cigpday}\OperatorTok{+}\NormalTok{agetv}\OperatorTok{+}\NormalTok{sex, }\DataTypeTok{data=}\NormalTok{fhstv_incMI)}
...
## 5 sex            -1.10      0.113      -9.65  4.79e-22
\end{verbatim}
...
##   term       hr low_hr high_hr
##   <chr>   <dbl>  <dbl>   <dbl>
## 1 cigpday  1.01  0.998    1.02
\end{verbatim}

We know see that the parameter coefficient for current smoking is attenuated, while the continuous \texttt{cigpday} variable has a positive parameter coefficient (HR=1.009 with each cigarette per day increase in smoking intensity), but the CI includes the null)
...
\NormalTok{coxph_modhospmitv5<-}\KeywordTok{coxph}\NormalTok{(}\KeywordTok{Surv}\NormalTok{(time, timemi2, hospmitv)}\OperatorTok{~}\StringTok{ }\NormalTok{smokingformer }\OperatorTok{+}\StringTok{ }\NormalTok{smokingcurrent}\OperatorTok{:}\NormalTok{cigpday}\OperatorTok{+}\NormalTok{agetv}\OperatorTok{+}\NormalTok{sex, }\DataTypeTok{data=}\NormalTok{fhstv_incMI) }\CommentTok{###leaving the dummy variable for 'never' out makes this the default comparison group}
...
## 4 smokingcurrent:cigpday   0.0159   0.00385     4.12  3.71e- 5
\end{verbatim}
...
\StringTok{  }\KeywordTok{tidy}\NormalTok{()}\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(term }\OperatorTok{==}\StringTok{ 'smokingcurrent:cigpday'}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{hr =} \KeywordTok{exp}\NormalTok{(}\DecValTok{20}\OperatorTok{*}\NormalTok{estimate),}
         \DataTypeTok{low_ci =} \DecValTok{20}\OperatorTok{*}\NormalTok{(estimate }\OperatorTok{-}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\NormalTok{std.error), }
         \DataTypeTok{high_ci =} \DecValTok{20}\OperatorTok{*}\NormalTok{(estimate }\OperatorTok{+}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\NormalTok{std.error), }
         \DataTypeTok{low_hr =} \KeywordTok{exp}\NormalTok{(low_ci), }
         \DataTypeTok{high_hr =} \KeywordTok{exp}\NormalTok{(high_ci)) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{select}\NormalTok{(term, hr, low_hr, high_hr)}
\end{Highlighting}
...
##   term                      hr low_hr high_hr
##   <chr>                  <dbl>  <dbl>   <dbl>
## 1 smokingcurrent:cigpday  1.37   1.18    1.60
\end{verbatim}
...
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{smokingint=}\KeywordTok{cut}\NormalTok{(cigpday, }\DataTypeTok{breaks=}\KeywordTok{c}\NormalTok{(}\OperatorTok{-}\DecValTok{1}\NormalTok{, }\FloatTok{0.1}\NormalTok{, }\DecValTok{5}\NormalTok{, }\DecValTok{10}\NormalTok{, }\DecValTok{20}\NormalTok{, }\OtherTok{Inf}\NormalTok{), }\DataTypeTok{labels=}\KeywordTok{c}\NormalTok{( }\StringTok{'None'}\NormalTok{, }\StringTok{'Light'}\NormalTok{, }\StringTok{'Moderate'}\NormalTok{, }\StringTok{'Heavy'}\NormalTok{, }\StringTok{'Very Heavy'}\NormalTok{)))}


\NormalTok{coxph_modhospmitv6<-}\KeywordTok{coxph}\NormalTok{(}\KeywordTok{Surv}\NormalTok{(time, timemi2, hospmitv)}\OperatorTok{~}\StringTok{ }\NormalTok{smokingformer }\OperatorTok{+}\StringTok{ }\NormalTok{smokingcurrent}\OperatorTok{:}\NormalTok{smokingint}\OperatorTok{+}\NormalTok{agetv}\OperatorTok{+}\NormalTok{sex, }\DataTypeTok{data=}\NormalTok{fhstv_incMI)}
...
## 8 smokingcurrent:smokingintVery Heavy   0.543    0.159       3.41   6.46e- 4
\end{verbatim}
...
\begin{verbatim}
## # A tibble: 0 x 4
## # ... with 4 variables: term <chr>, hr <dbl>, low_hr <dbl>, high_hr <dbl>
\end{verbatim}
...
\NormalTok{coxph_modhospmitv6 }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{tidy}\NormalTok{()}\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(term }\OperatorTok{==}\StringTok{ 'smokingcurrent:smokingintLight'}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{hr =} \KeywordTok{exp}\NormalTok{(estimate),}
         \DataTypeTok{low_ci =}\NormalTok{ (estimate }\OperatorTok{-}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\NormalTok{std.error), }
         \DataTypeTok{high_ci =}\NormalTok{ (estimate }\OperatorTok{+}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\NormalTok{std.error), }
         \DataTypeTok{low_hr =} \KeywordTok{exp}\NormalTok{(low_ci), }
...
##   term                              hr low_hr high_hr
##   <chr>                          <dbl>  <dbl>   <dbl>
## 1 smokingcurrent:smokingintLight 0.781  0.423    1.44
\end{verbatim}
...
\NormalTok{coxph_modhospmitv6 }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{tidy}\NormalTok{()}\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(term }\OperatorTok{==}\StringTok{ 'smokingcurrent:smokingintModerate'}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{hr =} \KeywordTok{exp}\NormalTok{(estimate),}
         \DataTypeTok{low_ci =}\NormalTok{ (estimate }\OperatorTok{-}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\NormalTok{std.error), }
         \DataTypeTok{high_ci =}\NormalTok{ (estimate }\OperatorTok{+}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\NormalTok{std.error), }
         \DataTypeTok{low_hr =} \KeywordTok{exp}\NormalTok{(low_ci), }
...
##   term                                 hr low_hr high_hr
##   <chr>                             <dbl>  <dbl>   <dbl>
## 1 smokingcurrent:smokingintModerate  1.06  0.658    1.71
\end{verbatim}
...
\NormalTok{coxph_modhospmitv6 }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{tidy}\NormalTok{()}\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(term }\OperatorTok{==}\StringTok{ 'smokingcurrent:smokingintHeavy'}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{hr =} \KeywordTok{exp}\NormalTok{(estimate),}
         \DataTypeTok{low_ci =}\NormalTok{ (estimate }\OperatorTok{-}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\NormalTok{std.error), }
         \DataTypeTok{high_ci =}\NormalTok{ (estimate }\OperatorTok{+}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\NormalTok{std.error), }
         \DataTypeTok{low_hr =} \KeywordTok{exp}\NormalTok{(low_ci), }
...
##   term                              hr low_hr high_hr
##   <chr>                          <dbl>  <dbl>   <dbl>
## 1 smokingcurrent:smokingintHeavy  1.91   1.47    2.48
\end{verbatim}
...
\NormalTok{coxph_modhospmitv6 }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{tidy}\NormalTok{()}\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(term }\OperatorTok{==}\StringTok{ 'smokingcurrent:smokingintVery Heavy'}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{hr =} \KeywordTok{exp}\NormalTok{(estimate),}
         \DataTypeTok{low_ci =}\NormalTok{ (estimate }\OperatorTok{-}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\NormalTok{std.error), }
         \DataTypeTok{high_ci =}\NormalTok{ (estimate }\OperatorTok{+}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\NormalTok{std.error), }
         \DataTypeTok{low_hr =} \KeywordTok{exp}\NormalTok{(low_ci), }
...
##   term                                   hr low_hr high_hr
##   <chr>                               <dbl>  <dbl>   <dbl>
## 1 smokingcurrent:smokingintVery Heavy  1.72   1.26    2.35
\end{verbatim}
...
\textbf{Explore the relationship between some of the other predictors in the dataset (specifically \texttt{bmi} and \texttt{sysbp}) and the hazard for MI hospitalization. Investigate whether the exposure response for these variable deviates from (log-)linearity using spline functions.}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{coxph_modhospmitvBMI<-}\KeywordTok{coxph}\NormalTok{(}\KeywordTok{Surv}\NormalTok{(time, timemi2, hospmitv)}\OperatorTok{~}\StringTok{ }\NormalTok{bmi}\OperatorTok{+}\NormalTok{agetv}\OperatorTok{+}\NormalTok{sex, }\DataTypeTok{data=}\NormalTok{fhstv_incMI)}
...
## 3 sex    -1.16     0.109      -10.6  1.75e-26
\end{verbatim}
...
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(splines)}
\NormalTok{coxph_modhospmitvBMIsp<-}\KeywordTok{coxph}\NormalTok{(}\KeywordTok{Surv}\NormalTok{(time, timemi2, hospmitv)}\OperatorTok{~}\StringTok{ }\KeywordTok{ns}\NormalTok{(bmi, }\DataTypeTok{df=}\DecValTok{3}\NormalTok{)}\OperatorTok{+}\NormalTok{agetv}\OperatorTok{+}\NormalTok{sex, }\DataTypeTok{data=}\NormalTok{fhstv_incMI)}
...
## 5 sex               -1.15     0.111     -10.4   4.12e-25
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{predbmi<-}\StringTok{ }\KeywordTok{termplot}\NormalTok{(coxph_modhospmitvBMIsp, }\DataTypeTok{se=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{plot=}\OtherTok{FALSE}\NormalTok{)}\OperatorTok{$}\NormalTok{bmi}
\NormalTok{predbmi }\OperatorTok{%>%}
\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ x, }\DataTypeTok{y =} \KeywordTok{exp}\NormalTok{(y))) }\OperatorTok{+}\StringTok{ }
\StringTok{  }\KeywordTok{geom_line}\NormalTok{() }\OperatorTok{+}\StringTok{ }
...
\includegraphics{adv_epi_analysis_files/figure-latex/unnamed-chunk-237-1.pdf}

We see that the relationship is mostly linear between values of about 25 and 40, but deviates from linearity at the lower and higher ends of the range in BMI. We would like to re-center this so that we are looking around a BMI value that is of \textbf{lowest} risk. The `normal' range of BMI is 20-25, so let's center out graph around 22.5. We will also add confidence bands to our plot.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{predbmi<-}\StringTok{ }\NormalTok{predbmi }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{center=}\KeywordTok{with}\NormalTok{(predbmi, y[x}\OperatorTok{==}\FloatTok{22.5}\NormalTok{]),}
         \DataTypeTok{ylow=}\NormalTok{predbmi}\OperatorTok{$}\NormalTok{y }\OperatorTok{-}\StringTok{ }\FloatTok{1.96}\OperatorTok{*}\NormalTok{predbmi}\OperatorTok{$}\NormalTok{se,}
         \DataTypeTok{yhigh=}\NormalTok{predbmi}\OperatorTok{$}\NormalTok{y }\OperatorTok{+}\StringTok{ }\FloatTok{1.96}\OperatorTok{*}\NormalTok{predbmi}\OperatorTok{$}\NormalTok{se)}

\NormalTok{predbmi }\OperatorTok{%>%}
\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ x, }\DataTypeTok{y =} \KeywordTok{exp}\NormalTok{(y}\OperatorTok{-}\NormalTok{center))) }\OperatorTok{+}\StringTok{ }
\StringTok{  }\KeywordTok{geom_line}\NormalTok{() }\OperatorTok{+}
\StringTok{  }
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{x =} \StringTok{"BMI"}\NormalTok{, }
       \DataTypeTok{y =} \StringTok{"Hazard Ratio"}\NormalTok{) }\OperatorTok{+}\StringTok{ }
...
\StringTok{  }\KeywordTok{geom_ribbon}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{ymin =} \KeywordTok{exp}\NormalTok{(ylow}\OperatorTok{-}\NormalTok{center), }\DataTypeTok{ymax =} \KeywordTok{exp}\NormalTok{(yhigh}\OperatorTok{-}\NormalTok{center)), }
...
\includegraphics{adv_epi_analysis_files/figure-latex/unnamed-chunk-238-1.pdf}
...
\StringTok{  }\KeywordTok{geom_ribbon}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{ymin =} \KeywordTok{exp}\NormalTok{(ylow}\OperatorTok{-}\NormalTok{center), }\DataTypeTok{ymax =} \KeywordTok{exp}\NormalTok{(yhigh}\OperatorTok{-}\NormalTok{center)), }
              \DataTypeTok{alpha=}\FloatTok{0.1}\NormalTok{, }
              \DataTypeTok{linetype=}\StringTok{"dashed"}\NormalTok{,}
              \DataTypeTok{color=}\StringTok{"grey"}\NormalTok{)}\OperatorTok{+}\KeywordTok{scale_y_continuous}\NormalTok{(}\DataTypeTok{trans=}\StringTok{'log2'}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{adv_epi_analysis_files/figure-latex/unnamed-chunk-239-1.pdf}
We now see that the HR is elevated as BMI increases until about BMI=40 with the effect estimate plateauing at higher values. The CI bands indicate a significant effect between BMI 25 and 42-43. Let's repeat the model, but now add a term for \texttt{sysbp}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{coxph_modhospmitvBMIBP<-}\KeywordTok{coxph}\NormalTok{(}\KeywordTok{Surv}\NormalTok{(time, timemi2, hospmitv)}\OperatorTok{~}\StringTok{ }\KeywordTok{ns}\NormalTok{(bmi, }\DataTypeTok{df=}\DecValTok{3}\NormalTok{)}\OperatorTok{+}\NormalTok{sysbp}\OperatorTok{+}\NormalTok{agetv}\OperatorTok{+}\NormalTok{sex, }\DataTypeTok{data=}\NormalTok{fhstv_incMI)}
...
## 6 sex               -1.24     0.112     -11.0   3.46e-28
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{coxph_modhospmitvBMIBPsp<-}\KeywordTok{coxph}\NormalTok{(}\KeywordTok{Surv}\NormalTok{(time, timemi2, hospmitv)}\OperatorTok{~}\StringTok{ }\KeywordTok{ns}\NormalTok{(bmi, }\DataTypeTok{df=}\DecValTok{3}\NormalTok{)}\OperatorTok{+}\KeywordTok{ns}\NormalTok{(sysbp, }\DataTypeTok{df=}\DecValTok{3}\NormalTok{)}\OperatorTok{+}\NormalTok{agetv}\OperatorTok{+}\NormalTok{sex, }\DataTypeTok{data=}\NormalTok{fhstv_incMI)}
...
## 8 sex                 -1.23     0.113     -10.9   7.10e-28
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{####We will go ahead and plot this as well using 110 as the center}
\NormalTok{predsysbp<-}\StringTok{ }\KeywordTok{termplot}\NormalTok{(coxph_modhospmitvBMIBPsp, }\DataTypeTok{se=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{plot=}\OtherTok{FALSE}\NormalTok{)}\OperatorTok{$}\NormalTok{sysbp }
\NormalTok{predsysbp<-}\StringTok{ }\NormalTok{predsysbp }\OperatorTok{%>%}\StringTok{  }
\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{center=}\KeywordTok{with}\NormalTok{(predsysbp, y[x}\OperatorTok{==}\DecValTok{110}\NormalTok{]),}
         \DataTypeTok{ylow=}\NormalTok{predsysbp}\OperatorTok{$}\NormalTok{y }\OperatorTok{-}\StringTok{ }\FloatTok{1.96}\OperatorTok{*}\NormalTok{predsysbp}\OperatorTok{$}\NormalTok{se,}
         \DataTypeTok{yhigh=}\NormalTok{predsysbp}\OperatorTok{$}\NormalTok{y }\OperatorTok{+}\StringTok{ }\FloatTok{1.96}\OperatorTok{*}\NormalTok{predsysbp}\OperatorTok{$}\NormalTok{se)}
...
\includegraphics{adv_epi_analysis_files/figure-latex/unnamed-chunk-241-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{coxph_modhospmitvall<-}\KeywordTok{coxph}\NormalTok{(}\KeywordTok{Surv}\NormalTok{(time, timemi2, hospmitv)}\OperatorTok{~}\StringTok{ }\NormalTok{smoking }\OperatorTok{+}\StringTok{ }\KeywordTok{ns}\NormalTok{(bmi, }\DataTypeTok{df=}\DecValTok{3}\NormalTok{)}\OperatorTok{+}\KeywordTok{ns}\NormalTok{(sysbp, }\DataTypeTok{df=}\DecValTok{3}\NormalTok{)}\OperatorTok{+}\NormalTok{agetv}\OperatorTok{+}\NormalTok{sex, }\DataTypeTok{data=}\NormalTok{fhstv_incMI)}
...
## 10 sex                 -1.15     0.115     -10.0   9.98e-24
\end{verbatim}
...
##   term              hr low_hr high_hr
##   <chr>          <dbl>  <dbl>   <dbl>
## 1 smokingcurrent  1.77   1.41    2.23
\end{verbatim}
...
\textgreater\textgreater\textgreater\textgreater\textgreater\textgreater\textgreater{} adfcbeb91b50102b3398a9ade88a1a9d52dff607
