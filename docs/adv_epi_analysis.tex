...
First, we can look at ``slices'', where we focus on just one lag value and look at the
non-linear relationship between temperature and mortality risk at that lag only. For example,
here are plots showing the relationship between temperature and mortality risk on the
same day of exposure (lag 0) and 21 days after exposure (lag 21):
...
relative risk using color for each combination). Here is the code to create those plots:
...
\includegraphics{adv_epi_analysis_files/figure-latex/unnamed-chunk-126-1.pdf}
