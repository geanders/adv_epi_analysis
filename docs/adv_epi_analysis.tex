...
Now we estimate the cumulative risk for a month of hot temperature (28 degrees C) compared to cooler temperature (7 degrees C) by essentially accumulating the RRs from each lag-day as follows:
...
If we exponentiate the model parameter we will get the cumulative RR estimate and likewise using the model parameter and standard error we can derive 95\% Confidence Intervals (CIs) for the cumulative RR
\[
RR=exp(\beta)=exp(-0.148)=0.86
\]
\[
95\% CIs: exp(\beta \pm 1.96*se) = exp(-0.148 \pm 1.96*0.05) = (0.78, 0.95)
\]

The \texttt{crosspred} object actually stores this information as well, so we can extract it directly without doing the math
...
Now let's calculate the cumulative RR for temperature at 28 degrees for the smooth lag-response function model from before:
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{hw_constant<-}\KeywordTok{crosspred}\NormalTok{(dl_basis_}\DecValTok{5}\NormalTok{, dist_lag_mod_}\DecValTok{6}\NormalTok{, }\DataTypeTok{at=}\KeywordTok{exphist}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\DecValTok{20}\NormalTok{,}\DecValTok{26}\NormalTok{), }\KeywordTok{rep}\NormalTok{(}\DecValTok{28}\NormalTok{, }\DecValTok{5}\NormalTok{)), }\DataTypeTok{times=}\DecValTok{31}\NormalTok{, }\DataTypeTok{lag=}\DecValTok{30}\NormalTok{) , }\DataTypeTok{cen =} \DecValTok{20}\NormalTok{, }\DataTypeTok{bylag =} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
## 31 0.1067468 0.08548286 0.06476856 0.04515363 0.02718775    0    0    0    0
...
## 31 1.112653 1.089243 1.066912 1.046189 1.027561    1    1    1    1    1     1
...
\includegraphics{adv_epi_analysis_files/figure-latex/unnamed-chunk-137-1.pdf}
...
\includegraphics{adv_epi_analysis_files/figure-latex/unnamed-chunk-158-1.pdf}
