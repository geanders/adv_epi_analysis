...
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{time_next=}\KeywordTok{lead}\NormalTok{(time), }\CommentTok{### bring forward the time from the next period}
         ...
         \DataTypeTok{agetv=}\NormalTok{age}\OperatorTok{+}\NormalTok{(time}\OperatorTok{/}\FloatTok{365.25}\NormalTok{)) }\OperatorTok{%>%}\StringTok{ }\CommentTok{### also create a time-varying age variable for age at beginning of each period}
