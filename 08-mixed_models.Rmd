...
fhs_sample <- fhs %>% 
  ...
  select(randid, sex, age, sysbp,cursmoke, bmi, educ, time) %>% 
...
specify---you can use the `glm` functionm since we're using only a fixed effect
...
need to specify `1` for an intercept:
...
`educ` fixed effect term. Based on the estimate, it looks like systolic blood
